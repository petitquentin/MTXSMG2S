...
do
    ...
    do
        ...
        { mpirun -n 4 ./generate_matrix.exe -D ${dim[${d}]} -M non-symm -L ${nb_diag[${nb}]} -P output/${dim[${d}]}_${nb_diag[${nb}]}_nnsymm.mtx  ; } > log/${dim[${d}]}_${nb_diag[${nb}]}_nnsymm.mtx 
        { mpirun -n 4 ./generate_matrix.exe -D ${dim[${d}]} -M non-herm -L ${nb_diag[${nb}]} -P output/${dim[${d}]}_${nb_diag[${nb}]}_nnsherm.mtx ; } > log/${dim[${d}]}_${nb_diag[${nb}]}_nnherm.mtx 
    done
    wait
done
